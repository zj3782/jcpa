...
(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            errormsg: 'Error Occurred', //错误提示信息
            usepager: false, //是否分页
            nowrap: true, //是否不换行
            ...
            onRowChecked: false,//当某行被选中的时候触发的函数
            onRowProp: false, //是否启用行的扩展事情功能
            extParam: {},
        ...
        var g = {
            ...
            changeSort: function(th) { //change sortorder
                ...
                var sorttype=$(th).attr('sort');
                
                if (p.onChangeSort){
                    p.onChangeSort(p.sortname, p.sortorder); 	
                }else if(typeof(sorttype)!='undefined'){
                	if(sorttype=='num')this.numSort(p.sortname, p.sortorder);
                	else this.asciiSort(p.sortname, p.sortorder);
                }else{
                    this.populate();
                }
            },
            /*以ASCII码顺序排序 zhujie add**/
            asciiSort:function(sortname,sortorder){
            	var index=this.indexOfColName(sortname);
            	if(index==-1)return;
            	sortorder = (sortorder=="desc")?0:1;
            	var rows = this.getRows();
            	//排序
            	var row;
            	for(var i=0;i<rows.length;i++){
            		for(var j=i+1;j<rows.length;j++){
            			if((sortorder && rows[i].cell[index]>rows[j].cell[index]) || (!sortorder && rows[i].cell[index]<rows[j].cell[index])){
            				row=rows[i];rows[i]=rows[j];rows[j]=row;
            			}
            		}
            	}
            	$("tr",t).remove();
            ...
            /*以数字顺序排序 zhujie add**/
            numSort:function(sortname,sortorder){
            	var index=this.indexOfColName(sortname);
            	if(index==-1)return;
            	sortorder = (sortorder=="desc")?0:1;
            	var rows = this.getRows();
            	...
            	for(var i=0;i<rows.length;i++){
            		for(var j=i+1;j<rows.length;j++){
            			c1=parseInt(rows[i].cell[index]);
            			c2=parseInt(rows[j].cell[index]);
            			if((sortorder && c1>c2) || (!sortorder && c1<c2)){
            				row=rows[i];rows[i]=rows[j];rows[j]=row;
            			}
            		}
            	}
            ...
            populate: function() { //get latest data 
                ...
                $.ajax({
                    ...
                    error: function(data) { try { if (p.onError) { p.onError(data); } else { alert("Data Access Fail.") } g.hideLoading(); } catch (e) { } }
                });
            },
            doSearch: function() {
            ...
            getCheckedRows: function() {//获取选中的行
            	...
            	$("tr",t).each(function() {
            		...
            		$("td",this).each(function(){
                    	if(!$(this).hasClass("chboxtd")){//不是checkbox
                    		row.cell.push($("div",this).html());
                    	}
                    });
                    rows.push(row);
                });
            ...
            /**获取表头display zhujie add*/
            getColHeader:function(){
            	var ths = $('thead tr:first th', g.hDiv);
            	var header=[];
            	$(ths).each(function(j){
            		if(!$(this).hasClass("chboxtd")){//不是checkbox
            			header.push($("div",this).html());
                	}
                });
            	return header;
            },
            ...
            getColNames:function(){
            	...
            	$(ths).each(function(j){
            		if($(this).attr("cname")){
            			name.push($(this).attr("cname"));
                	}
                });
            	return name;
            },
            /**找出col列的位置**/
            indexOfColName:function(col){
            	var name=this.getColNames();
            	if(!name || typeof(name)!='object')return -1;
            	for(var i=0;i<name.length;i++){
            		if(name[i]==col)return i;
            	}
            	return -1;
            },
            /**通过ids获取rows zhujie add*/
            getRowsByIds:function(ids){
            	var rows=[];
            	for(var i=0;i<ids.length;i++){
                    var row ={id:ids[i],cell:[]};
                    var td=$("#row"+ids[i]+" td",t);
                    $(td).each(function(j){
                    	if(!$(this).hasClass("chboxtd")){//不是checkbox
                    		row.cell.push($("div",this).html());
                    	}
        ...
        if (p.colModel) {
            ...
            if (p.showCheckbox) {
                ...
                cth.addClass("cth").attr({ 'axis': "col-1", width:"23", "isch": true }).append(cthch);
                $(tr).append(cth);
            }
            for (i = 0; i < p.colModel.length; i++) {
                ...
                if(cm.name)$(th).attr('cname', cm.name);//zhujie add
                if (cm.name && cm.sortable){
                	$(th).attr('abbr', cm.name);
                	if(cm.sorttype)$(th).attr('sort', cm.sorttype);//zhujie add
                } 
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<div class="pGroup"><div class="pFirst pButton" title="First Page"><span></span></div><div class="pPrev pButton" title="Previous Page"><span></span></div> </div><div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">Page <input type="text" size="1" value="1" /> of <span> 1 </span></span></div><div class="btnseparator"></div><div class="pGroup"> <div class="pNext pButton" title="Next Page"><span></span></div><div class="pLast pButton" title="Last Page"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="Reload"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div>';
            ...
            if (p.useRp) {
                ...
                $('.pDiv2', g.pDiv).prepend("<div class='pGroup'>One Page<select name='rp'>" + opt + "</select>Records</div> <div class='btnseparator'></div>");
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'>FastSearch：<input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt + "</select> <input type='button' name='qclearbtn' value='Clear' /></div>");
    ...
    $.fn.flexChangePage = function(type) {
        return this.each(function() {
            if (this.grid) {
    ...
    $.fn.flexGetRowsByIds = function(ids){//通过ids获取rows
    	if (this[0].grid) {
            return this[0].grid.getRowsByIds(ids);
        }
        return [];
    };
    $.fn.flexGetRows = function(){//获取rows
    	if (this[0].grid) {
    ...
    $.fn.flexUpdateData = function(data) { // function to add data to grid
    ...
    $.fn.flexAddRowsData=function(rows){//add row(s) to grid，zhujie add
    	return this.each(function() {
            if (this.grid) this.grid.addRowsData(rows);
        });
    };
    
    $.fn.flexUpdateRowData=function(row){//update row data，zhujie add
    	if (this[0].grid) {
            return this[0].grid.updateRowData(row);
        }
    	return false;
    };
    
    $.fn.flexRowsCount=function(){//get row count，zhujie add
    	if (this[0].grid) {
            return this[0].grid.getRowsCount();
        }
        return 0;
    };
    
    $.fn.flexDelRowsByIds=function(ids){//delete rows by ids
    	if (this[0].grid) {
            this[0].grid.delRowsByIds(ids);
        }
    };
    
    $.fn.flexGetColHeader=function(){//获取表头元素显示内容
    	if (this[0].grid) {
            return this[0].grid.getColHeader();
        }
    	return [];
    };
    
    $.fn.flexGetColName=function(){//获取表头元素名称
    	if (this[0].grid) {
            return this[0].grid.getColNames();
        }
    	return [];
    };
    
    $.fn.flexAsciiSort=function(sortname,sortorder){//以ASCII排序
    	if (this[0].grid) {
            this[0].grid.asciiSort(sortname,sortorder);
        }
    };

    $.fn.flexNumSort=function(sortname,sortorder){//以num排序
    	if (this[0].grid) {
            this[0].grid.numSort(sortname,sortorder);
        }
    };
    
    $.fn.flexRefreshMenuPop=function(){
    	if (this[0].grid) {
            this[0].grid.addRowProp();
        }
    };
